...
%------------------------IMPORTANTE---------------------------------------
\newif\iffinal
\finaltrue % <-- MANTIDO FALSE PARA REVISÃO CEGA (BLIND REVIEW)
%------------------------------------------------------------------------- 
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.75\columnwidth]{LSTM.png}
    \vspace{-0.4cm}
    \caption{Representação da célula LSTM
    \cite{ghojogh2023recurrentneuralnetworkslong}.}
    \label{fig:lstm}
\end{figure}

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.75\columnwidth]{GRU.png}
    \vspace{-0.4cm}
    \caption{Representação da célula GRU
    \cite{ghojogh2023recurrentneuralnetworkslong}.}
    \label{fig:celula_gru}
\end{figure}
...
\centering
\caption{Parâmetros utilizados no modelo \textit{LSTM}.}
\label{tab:parametros_lstm}
\small
\begin{tabular}{
>{\bfseries\raggedright\arraybackslash}p{0.34\columnwidth}|
...
Escopo do modelo
& Uma rede independente por localidade \\
\hline

Período histórico
& 2016--2025, totalizando 120 meses \\
\hline
...
Treinamento(n)
& 84 janelas por localidade \\
\hline

Conjunto de teste
& 12 meses finais da série \\
\hline
...
& Resíduo normalizado e 12 indicadores mensais \\
\hline

Pré-processamento
& Tendência quadrática e normalização $Z$-score \\
\hline
...
Otimizador
& Adam, com taxa de aprendizado $0{,}01$ \\
\hline
...
Horizonte futuro
& 72 meses, com previsão recursiva \\
\hline

Histórico das perdas
& Não persistido pelo script \\
\hline\hline
\end{tabular}
\end{table}

As redes neurais recorrentes foram treinadas utilizando janelas deslizantes de 24 meses. A implementação LSTM ajustou uma rede independente para cada localidade, enquanto a arquitetura GRU foi treinada globalmente, incorporando uma codificação \textit{one-hot} para identificar cada localidade. Em ambas as arquiteturas, a camada recorrente possui 32 unidades e é seguida por uma camada densa com uma unidade, responsável pela previsão de um único passo temporal.

A série histórica utilizada pelas redes compreendeu o período de 2016 a 2025, totalizando 120 observações mensais, quando não considerados eventuais meses ausentes. A aplicação da janela de 24 meses produziu 96 janelas supervisionadas por localidade, das quais 84 foram destinadas ao treinamento e 12 foram reservadas para o teste temporal. As previsões futuras foram obtidas recursivamente, um mês por vez, até o horizonte de 72 meses.

\begin{table}[H]
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.25}
\centering
\caption{Parâmetros utilizados no modelo \textit{GRU} global.}
\label{tab:parametros_gru}
\small
\begin{tabular}{
>{\bfseries\raggedright\arraybackslash}p{0.34\columnwidth}|
>{\raggedright\arraybackslash}p{0.54\columnwidth}
...
Escopo do modelo
& Uma única rede global para todas as localidades \\
\hline
...
& 84 janelas por localidade, concatenadas no tensor global \\
\hline
...
Dimensão da entrada
& $24 \times (13 + N_{\mathrm{loc}})$ \\
\hline

Variáveis de entrada
& Ruído normalizado, 12 indicadores mensais e indicadores da localidade \\
\hline

Pré-processamento
& Tendência quadrática, perfil sazonal mensal e normalização do ruído \\
\hline
...
O treinamento foi realizado por meio da função de perda erro quadrático médio (MSE), utilizando o otimizador Adam com taxa de aprendizado igual a 0,01. A rede LSTM foi treinada durante 100 épocas, com lotes de oito janelas, enquanto a rede GRU foi treinada durante 50 épocas, com lotes de 32 janelas. Em ambos os casos, 10\% das janelas de treinamento foram utilizadas para validação interna.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.95\linewidth]{PainelResumo.png}
